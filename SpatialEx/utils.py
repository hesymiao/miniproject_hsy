...
def create_ImageEncoder(model_name='resnet50', pretrained=True, frozen=True):
    ...
    if model_name == 'resnet50':
        ...
        local_dir = "/bigdat2/user/hesy/mini_project/vit_large_patch16_224.dinov2.uni_mass100k/" #"../assets/ckpts/vit_large_patch16_224.dinov2.uni_mass100k/"
        ...
        if timm is None:
            raise ImportError("timm is required for the 'gigapath' image encoder but is not installed.")
        model = timm.create_model("hf_hub:prov-gigapath/prov-gigapath", pretrained=True)
    elif model_name == 'phikon':
        if ViTModel is None:
            raise ImportError("transformers is required for the 'phikon' image encoder but is not installed.")
