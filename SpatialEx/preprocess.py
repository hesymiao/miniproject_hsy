...
def _tile_he_patches_from_coords(resolution, coords, img):
    patch_radius = int(resolution / 2.0)
    he_patches = [0] * coords.shape[0]
    for i in tqdm(range(coords.shape[0])):
        x, y = coords[i]
        he_patches[i] = torch.tensor(img[y - patch_radius: y + patch_radius, x - patch_radius:x + patch_radius])
    return torch.stack(he_patches, dim=0) / 255.0


def Tiling_HE_patches_multiscale(resolutions, adata, img, key='image_coor'):
    print('======================== Tiling HE patches for each single cells ===========================')
    resolutions = [int(r) for r in resolutions]
    max_resolution = max(resolutions)
    patch_radius = int(max_resolution / 2.0)
    print('max patch radius is ', patch_radius)

    outlier_cells = np.unique(np.where(adata.obsm[key] < patch_radius)[0])
    if len(outlier_cells) != 0:
        print(f'Remove the outlier cells: {len(outlier_cells)} cells filtered, and Anndata file was reduced!')
        inlier_cells = set(np.arange(adata.n_obs)) - set(outlier_cells)
        adata = adata[list(inlier_cells)]
    ...
    for resolution in resolutions:
        print(f'Building patches for resolution {resolution}')
        he_patches.append(_tile_he_patches_from_coords(resolution, coords, img))
    return he_patches, adata
...
def Extract_HE_patches_representaion_multiscale(
    he_patches_dict,
    adata=None,
    store_key='he',
    image_encoder='uni',
    ...
    feats = []
    if isinstance(he_patches_dict, dict):
        items = list(he_patches_dict.values())
    else:
        items = list(he_patches_dict)
    for idx, he_patches in enumerate(items):
        tmp_key = f'__tmp_he_{idx}'
        adata = Extract_HE_patches_representaion(
            he_patches,
            store_key=tmp_key,
            adata=adata,
        ...
        feats.append(adata.obsm.pop(tmp_key))
    adata.obsm[store_key] = np.concatenate(feats, axis=1)
    return adata
...
def normalize_graph(H, edge_weight=None, norm_type='gcn'):
    if norm_type == 'row':
        normalization_factors = sp.csr_matrix(1.0 / H.sum(1))  # 行归一化
        adj = H.multiply(normalization_factors)
    elif norm_type == 'col':
        normalization_factors = sp.csr_matrix(1.0 / H.sum(0))  # 列归一化
        adj = H.multiply(normalization_factors)
    elif norm_type == 'both':
        normalization_factors1 = sp.csr_matrix(1.0 / H.sum(0))  # 列归一化
        normalization_factors2 = sp.csr_matrix(1.0 / H.sum(1))  # 行归一化
        adj = H.multiply(normalization_factors1)
        adj = adj.multiply(normalization_factors2)
    elif norm_type == 'gcn':
