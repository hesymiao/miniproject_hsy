...
class SpatialEx:
    ...
    def auto_inference(self):
        ...
        panel_1b.columns = self.adata2.var_names
        ...
        panel_2a.columns = self.adata1.var_names
        panel_2a = panel_2a.values

        if self.save_path is not None:
            os.makedirs(self.save_path, exist_ok=True)
            np.save(os.path.join(self.save_path, 'B1.npy'), panel_1b)
            np.save(os.path.join(self.save_path, 'A2.npy'), panel_2a)
            print(f'The results have been sucessfully saved in {self.save_path}')      # 改成保存路径
